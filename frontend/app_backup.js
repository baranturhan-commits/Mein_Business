...
document.addEventListener('DOMContentLoaded', () => {
    console.log('­ƒÜÇ Initializing Mein Business Dashboard...');
    loadDashboard();
    updateLastUpdate();
...
function createMandantCard(mandant) {
    ...
    if (offen > 0) {
        // Hier k├Ânnte man noch pr├╝fen wir alt die Rechnungen sind (wenn Backend das liefert)
        // F├╝r jetzt: Offen > 0 ist Gelb, Offen > 1000 ist Rot (als Beispiel)
        if (offen > 1000) {
...
async function refreshData() {
    console.log('­ƒöä Refreshing dashboard...');
    await loadDashboard();
    updateLastUpdate();
}
...
function openBackend(module) {
    ...
    alert(`Terminal ├Âffnen und ausf├╝hren:\n\n${command}`);
    // In einer erweiterten Version k├Ânnten hier WebSockets verwendet werden
}
...
function showError(message) {
    ...
    grid.innerHTML = `
        <div class="loading">
            <p style="color: var(--danger);">ÔØî ${message}</p>
        </div>
    `;
}
...
async function submitMandant(event) {
    ...
    statusDiv.innerHTML = '<p>­ƒÅó Erstelle Mandanten...</p>';
    ...
    try {
        ...
        if (result.success) {
            statusDiv.innerHTML = '<p class="success">Ô£à Mandant erfolgreich angelegt!</p>';
            setTimeout(() => {
                closeMandantModal();

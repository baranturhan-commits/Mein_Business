"""
API Server f├╝r Mein Business Admin Dashboard
Flask-basierte REST API f├╝r Frontend-Zugriff
"""
...
def get_mandant_stats(mandant_id):
    """Detaillierte Statistiken f├╝r einen Mandanten"""
    try:
        ...
        logger.error(f"Fehler beim Laden der Stats f├╝r {mandant_id}: {str(e)}")
        return jsonify({'error': str(e)}), 500

def get_einnahmen_stats(mandant_dir):
    """Statistiken aus einnahmen.xlsx oder einnahmen.csv"""
    # Pr├╝fe xlsx in Einnahmen/
...
def get_ausgaben_stats(mandant_dir):
    ...
    # Z├ñhle einfach die Anzahl der Belege
    count = sum(1 for _ in ausgaben_dir.rglob('*.*') if _.is_file())
    
    return {
        'anzahl': count,
        'total': 0  # K├Ânnte sp├ñter aus CSV gelesen werden
    }
...
def get_dashboard():
    """Dashboard-├£bersicht ├╝ber alle Mandanten"""
    try:
...
def get_mandant_rechnungen(mandant_id):
    """Liste aller Rechnungen f├╝r einen Mandanten"""
    try:
        mandant_dir = MANDANTEN_DIR / mandant_id
        rechnungen_dir = mandant_dir / 'Rechnungen'
...
def upload_mandant_logo(mandant_id):
    ...
    try:
        ...
        if not file.filename:
            return jsonify({'error': 'Keine Datei ausgew├ñhlt'}), 400
...
def handle_mitarbeiter(mandant_id):
    """Verwalte Mitarbeiter (Liste lesen & hinzuf├╝gen/bearbeiten)"""
    mandant_dir = MANDANTEN_DIR / mandant_id
...
def get_employee_payslips(mandant_id, emp_id):
    """Liefere Liste aller Lohnabrechnungen f├╝r einen Mitarbeiter"""
    try:
        mandant_dir = MANDANTEN_DIR / mandant_id
        json_file = mandant_dir / 'mitarbeiter.json'
...
def check_duplicate_invoice(mandant_id):
    """Pr├╝ft auf ├ñhnliche Rechnungen (Duplikat-Warnung)"""
    try:
        data = request.get_json()
...
def update_invoice_status(mandant_id, rechnung_nr):
    ...
    try:
        ...
        logger.info(f"Status aktualisiert: {mandant_id}/{rechnung_nr} ÔåÆ {new_status}")
...
def create_mandant():
    ...
    try:
        ...
        import re
        safe_name = firma.replace('├ñ', 'ae').replace('├Â', 'oe').replace('├╝', 'ue').replace('├ƒ', 'ss')
        safe_name = safe_name.replace('├ä', 'Ae').replace('├û', 'Oe').replace('├£', 'Ue')
...
def process_recurring(mandant_id):
    """Manuell Abos pr├╝fen und ausf├╝hren"""
    try:
        import recurring_invoices
        mandant_dir = MANDANTEN_DIR / mandant_id
...
def add_preisliste_position(mandant_id):
    """F├╝gt eine Position zur Preisliste hinzu"""
    try:
        data = request.get_json()
        ...
        logger.info(f"Preislisten-Position hinzugef├╝gt: {mandant_id}/{bezeichnung}")
        return jsonify({'success': True, 'position': position_data})
    
    except Exception as e:
        logger.error(f"Fehler beim Hinzuf├╝gen der Position: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def delete_preisliste_position(mandant_id, pos):
    """L├Âscht eine Preislisten-Position"""
    try:
        mandant_dir = MANDANTEN_DIR / mandant_id
        preisliste_file = mandant_dir / 'preisliste.xlsx'
        ...
        logger.info(f"Position {pos} gel├Âscht: {mandant_id}")
        return jsonify({'success': True})
    
    except Exception as e:
        logger.error(f"Fehler beim L├Âschen: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def confirm_import_preisliste(mandant_id):
    """Best├ñtigt und speichert importierte Positionen"""
    try:
        data = request.get_json()
...
def export_month(mandant_id):
    """Erstellt ein ZIP-Archiv f├╝r den Steuerberater (Monatsabschluss)"""
    try:
        data = request.get_json()
        ...
        # 1. Sammle Daten
        # Zeitraum definieren (String Matching f├╝r Einfachheit: "MM.YYYY" oder "YYYY-MM")
        ...
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            # --- EINNAHMEN ---
            # Wir ben├Âtigen die Rechnungs-PDFs und eine Liste
            import excel_utils
    ...
    try:
        ...
        for it in raw_items:
            try:
                ...
                # Format: "1.0 Stk x Bezeichnung (50.00Ôé¼)"
                desc = f"{m} {it.get('einheit', 'Stk')} x {it.get('bezeichnung')} (EP: {p:.2f}Ôé¼)"
    ...
    try:
        ...
        if not data:
            data = {
                ...
                'firma': 'Unbekannt',
                'kategorie': category,
                'beschreibung': 'Manueller Check n├Âtig'
            }
        ...
        return jsonify({
            'success': True,
            'message': f"Beleg erfasst: {data.get('betrag')}Ôé¼ ({data.get('firma')})",
            'data': data
        })
...
def run_scheduler():
    ...
    logger.info("ÔÅ░ Scheduler gestartet (Backup t├ñglich um 23:00)")
    last_run = None
    ...
    while True:
        ...
        if now.hour == 23 and now.minute == 0:
            ...
            if last_run != today:
                try:
                    logger.info("ÔÅ░ Starte automatisches Backup...")
                    import backup
...
if __name__ == '__main__':
    # Restart Trigger
    logger.info("­ƒÜÇ Starting API Server...")
    print("\n" + "="*50)
    print("­ƒÜÇ Mein Business API Server")
    print("="*50)
    print("­ƒôí API l├ñuft auf: http://localhost:5000")
    print("­ƒôè Dashboard: http://localhost:5000/api/dashboard")
    print("­ƒÅó Mandanten: http://localhost:5000/api/mandanten")
    print("\n­ƒÆí ├ûffne das Frontend in deinem Browser")
    print("="*50 + "\n")
